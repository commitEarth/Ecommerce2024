...
export const myOrder=TryCatch(async(req,res,next)=>{
    const {id:user} = req.query; //taking user as the primitive 
...
export const newOrder=TryCatch(async(req:Request<{},{},NewOrderRequestBody>,res:Response,next:NextFunction)=>{
    ...
    invalidateCache({
        ...
        admin:true,
        userId:user,
        productId:order.orderItems.map(i=>String(i.productId)) ///////return an array 
    }); 
...
export const processOrder=TryCatch(async(req,res,next)=>{
    ...
    invalidateCache({product:false,order:true,admin:true,userId:order.user,orderId:String(order._id)}) //order krne se product change na ho 
...
export const deleteOrder=TryCatch(async(req,res,next)=>{
    const  {id}=req.params;    //------------------------------------------------>using just id instead of {id} is wrong 
